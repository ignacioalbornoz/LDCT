...
from config_sketch import config
...
# Use the new SketchDataset for sketch training
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    for epoch in range(config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            clean_images = batch["target"]  # Sketch images (target)
            ...
            # If concatenation, concatenate noisy image with edge image (conditioning)
            if config.conditioning == "concatenate":
                edge_images = batch["image"]  # Edge images (conditioning)
                noisy_images = torch.cat((noisy_images, edge_images), dim=1)
...
def evaluate(config, epoch, pipeline, images):
    ...
    expected_images = config.eval_batch_size
    actual_images = len(images)
    
    if actual_images < expected_images:
        print(f"WARNING: Expected {expected_images} images but got {actual_images} images at epoch {epoch}")
        # Adjust grid dynamically based on actual number of images
        if actual_images <= 4:
            rows, cols = 1, actual_images
        else:
            cols = 4
            rows = (actual_images + cols - 1) // cols  # Ceiling division
    else:
        # Use expected grid for batch of 4
        rows = config.eval_batch_size // 4
        cols = 4
